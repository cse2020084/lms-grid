...
export class RoleComponent implements OnInit {
  public roleList$: BehaviorSubject<Array<any>> = new BehaviorSubject([]);
  constructor(
  ...
  ngOnInit(): void {
    ...
    this.getRoleList(false).then((records: any) => {
      
      this.roleList$.next(records);
  ...
  */
  onGridReady(params) {
    ...
    getRoleList(auditFlag: boolean) {
      return new Promise((resolve, reject) => {
        this.loader.showSpinner('Data list on the way.');
        const payload = {
        ...
        this.dataService.retrieveData('roleservice/getAllRole/', payload).subscribe((records: any) => {
   ...
   onSaveRow(item: any): void {
    ...
    updateRow(item){
      ...
      this.dataService.saveRowData(item, 'roleservice/updateRole/', payload).then((result:any) => {
    ...
    activateRow(item){
      ...
      this.dataService.saveRowData(item,'roleservice/activateRole',payload).then((result:any) => {
        if (result && result.entityID) item.entityID = result.entityID
        item.activeFlag = 1;
    ...
    deactivateRow(item){
      ...
      this.dataService.saveRowData(item,'roleservice/deactivateRole',payload).then((result:any) => {
        if (result && result.entityID) item.entityID = result.entityID
        item.activeFlag = 0;
        this.gridApi.applyTransaction({ update: [item] });
        this.gridApi.refreshCells({ force: true });
...
/***********/
  ...
  public logDefs:CustomColDef[]=[
    {headerName: 'Role', field: 'entityBusinessName',required:true},
  ...
  showLog(toggleFlag) {
    ...
    if(toggleFlag){
      this.getRoleList(true).then((records:any)=>{
        this.roleList$.next(records)
      })
      .catch(error=>{
        this.roleList$.next([]);
     ...
     // for export in excel and csv
          getDownload(format: string) {
            ...
            if (format.toLowerCase() === 'excel') {
              ...
              exportParams.fileName = 'RoleList.xlsx'; // Optional: Customize file name
              ...
              exportParams.fileName = 'RoleList.csv'; // Optional: Customize file name
          ...
          onBtPrint() {
            ...
            generatePDF(gridRowData, selectedHeaders,'Role');
