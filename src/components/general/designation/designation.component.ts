...
export class DesignationComponent implements OnInit {
  public designationList$: BehaviorSubject<Array<any>> = new BehaviorSubject([]);
  constructor(
  ...
  ngOnInit(): void {
    ...
    this.getDesignationList(false).then((records: any) => {
      this.departmentService.setDepartmentList(records); 
      this.designationList$.next(records);
  ...
  public columnDefs: CustomColDef[] = [
        ...
        {
          headerName: 'Designation', sortable:true,
        ...
        {
          headerName: 'Abbreviation',sortable:true,
        ...
        {
          headerName: 'Last Updated By',sortable:true,
          field: 'createdByUser',
          minWidth:200,
          required:false,
  ...
  */
  onGridReady(params) {
    ...
    //   getRowClass: (params) => {
    //     return params.data.activeFlag === 0 ? 'deactivated-row' : '';
    // },
      ...
      if (params.data && params.data.activeFlag !== 1) {
    ...
    getDesignationList(auditFlag: boolean) {
      return new Promise((resolve, reject) => {
        ...
        this.dataService.retrieveData('designationservice/getAllDesignation/', payload).subscribe((records: any) => {
   ...
   onSaveRow(item: any): void {
    ...
    updateRow(item){
      ...
      this.dataService.saveRowData(item, 'designationservice/updateDesignation/', payload).then((result) => {
    ...
    activateRow(item){
      ...
      this.dataService.saveRowData(item,'designationservice/activateDesignation',payload).then((result:any) => {
    ...
    deactivateRow(item){
      ...
      this.dataService.saveRowData(item,'designationservice/deactivateDesignation',payload).then((result:any) => {
        if (result && result.entityID) item.entityID = result.entityID
        item.activeFlag = 0;
        this.gridApi.applyTransaction({ update: [item] });
        this.gridApi.refreshCells({ force: true });
...
/***********/
  ...
  public logDefs:CustomColDef[]=[
    {headerName: 'Designation', field: 'entityBusinessName',required:true},
  ...
  showLog(toggleFlag) {
    ...
    if(toggleFlag){
      this.getDesignationList(true).then((records:any)=>{
        this.designationList$.next(records)
      })
      .catch(error=>{
        this.designationList$.next([]);
     ...
     // for export in excel and csv
          getDownload(format: string) {
            ...
            if (format.toLowerCase() === 'excel') {
              ...
              exportParams.fileName = 'DesignationList.xlsx'; // Optional: Customize file name
              ...
              exportParams.fileName = 'DesignationList.csv'; // Optional: Customize file name
          ...
          onBtPrint() {
            ...
            generatePDF(gridRowData, selectedHeaders,'Designation');
